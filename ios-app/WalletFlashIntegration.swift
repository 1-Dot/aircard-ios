        ...
        Task.detached { [weak self] in
            ...
            for (i, card) in selected.enumerated() {
                ...
                guard case .valid(let manifest) = verifiedBackup else {
                    if backupPreparation.movedOriginalFiles {
                        _ = await Self.restoreOriginalArtworkBackupFiles(
                            cardId: cleanId,
                            pairingPath: pairingPath,
                ...
                guard writeResult.ok else {
                    ...
                    if backupPreparation.movedOriginalFiles {
                        await MainActor.run {
                            self.cardFlashLog.append("  ↩️ Restoring original artwork after failed skin write…")
                        }
                        ...
                        if rollback.ok {
                            ...
                            await MainActor.run {
                                self.cardFlashLog.append("  ✅ Original artwork restored")
                            }
                        } else {
                            await MainActor.run {
