...
extension AppViewModel {
    ...
    func restoreOriginalArtworkExactly(for cardId: String) {
        ...
        Task.detached { [weak self] in
            ...
            await MainActor.run {
                self.cardFlashLog.append("  ✅ Current artwork snapshot: \(snapshotCount) file\(snapshotCount == 1 ? "" : "s")")
            }
            ...
            if snapshotFailed {
                ...
                await MainActor.run {
                    self.cardFlashPhase = .done(ok: false)
                    self.cardFlashLog.append("  ❌ Could not snapshot current artwork: \(snapshotFailure ?? "unknown error")")
                    if let rollbackError {
                        self.cardFlashLog.append("  ❌ Snapshot rollback also failed: \(rollbackError)")
                        if recoveryURL != nil {
                            self.cardFlashLog.append("  🛟 Recovery snapshot retained for manual repair")
                        }
                    } else if snapshotCount > 0 {
                        self.cardFlashLog.append("  ✅ Current artwork restored after snapshot failure")
                    }
                    WalletCardOperationStore.shared.set(.failed("Restore snapshot failed"), for: cleanId)
                    self.errorMessage = "Could not safely prepare the original-artwork restore."
            ...
            guard restore.ok else {
                ...
                await MainActor.run {
                    self.cardFlashPhase = .done(ok: false)
                    self.cardFlashLog.append("❌ Original-artwork write failed: \(restore.error ?? "unknown write error")")
                    if let rollbackError {
                        self.cardFlashLog.append("❌ Could not restore the pre-restore artwork snapshot: \(rollbackError)")
                        if recoveryURL != nil {
                            self.cardFlashLog.append("🛟 Pre-restore snapshot retained in Recovery diagnostics")
                        }
                    } else if snapshotCount > 0 {
                        self.cardFlashLog.append("✅ Previous artwork restored after failed original restore")
                    }
                    WalletCardOperationStore.shared.set(.failed("Original restore failed"), for: cleanId)
                    self.errorMessage = "Could not restore the original Wallet artwork."
                    self.objectWillChange.send()
                }
                return
            }
            ...
            await MainActor.run {
                ...
                self.cardFlashLog.append("  🧹 Original set written; invalidating Wallet cache…")
            }
            ...
            await MainActor.run {
                ...
                self.cardFlashLog.append("✅ Exact original Wallet artwork restored (\(snapshotCount) current → \(manifest.files.count) original). Force-close Wallet to refresh.")
